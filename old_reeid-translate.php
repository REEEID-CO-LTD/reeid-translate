...
// REEID: ensure only the bridge prints hreflang on products
 ...
 * decode percent-encoding, trim, and ensure a non-empty fallback.
 */
if (! function_exists('reeid_sanitize_native_slug')) {
    function reeid_sanitize_native_slug($slug)
    {
        ...
        // Normalize whitespace and replace sequences of invalid chars with hyphen.
        // Allow Unicode letters/numbers and hyphen.
        $slug = preg_replace('/[^\p{L}\p{N}\-]+/u', '-', $slug);
        $slug = preg_replace('/-+/', '-', $slug);
        $slug = trim($slug, '-');

        // Ensure non-empty fallback; use wp_generate_uuid4 if available, otherwise uniqid()
        if ($slug === '') {
            if (function_exists('wp_generate_uuid4')) {
                $slug = 'translated-' . wp_generate_uuid4();
            } else {
                $slug = 'translated-' . uniqid();
            }
        }

        // Optional: trim to reasonable length for slugs
...
// Expose plugin master language map for front-end renderers (read-only).
if (!isset($GLOBALS['reeid_languages']) || !is_array($GLOBALS['reeid_languages'])) {
    $GLOBALS['reeid_languages'] = $langs;
}

        // Sort by label (A→Z), case-insensitive
...
/**
 * Return language-to-flag ASSET mapping (for /assets/flags/*.svg),
 * not emoji. Values are ISO country codes for flag filenames.
 * Keys must match canonical language codes above.
 */
if (! function_exists('reeid_get_language_flags')) {
    function reeid_get_language_flags()
    {
        return array(
            ...
            'de'    => 'de', // Germany
            'el'    => 'gr', // Greece
            'en'    => 'us', // United States (generic English)
            'es'    => 'es', // Spain
            'fa'    => 'ir', // Iran (Persian)
            'fi'    => 'fi', // Finland
            'fr'    => 'fr', // France
...
if (! function_exists('reeid_get_allowed_languages')) {
    function reeid_get_allowed_languages()
    {
        ...
        $free = array('en', 'es', 'fr', 'de', 'zh', 'ja', 'ar', 'ru', 'th', 'it-it' /* or 'pt-pt' instead of 'it-it' if you want */);
...
function reeid_render_settings_tab()
{
    ...
    // Allow override of the privacy URL via filter if needed.
    $privacy_url = apply_filters('reeid_privacy_policy_url', 'https://reeid.com/privacy-policy/');
    ?>
    <form method="post" action="options.php">
        ...
        submit_button();
        ?>
        <p class="description reeid-privacy-note" style="margin-top:8px;">
            <?php echo esc_html__('By saving, the plugin may contact go.reeid.com for license checks and api.reeid.com for rules/translation services.', 'reeid-translate'); ?>
            <a href="<?php echo esc_url($privacy_url); ?>" target="_blank" rel="noopener">
                <?php echo esc_html__('Privacy Policy', 'reeid-translate'); ?>
            </a>
        </p>
    </form>
    <?php
...
function reeid_handle_validate_license_key()
{
    ...
    // Security.
    check_ajax_referer('reeid_translate_nonce_action', 'nonce');
...
/*==============================================================================
  SECTION 12.A — Feature Flag Shim (PHP 7.x safe, no API calls yet)
  - Goal: provide stable helpers to gate PRO UI/actions without altering behavior.
  - Current behavior: considers license "active" if a license key is saved.
  - Later: we’ll swap internals to call api.reeid.com, names stay the same.
==============================================================================*/

/**
 * Return true if license appears active (shim version).
 * For now: active == license key exists (non-empty).
 */
if (!function_exists('reeid_license_is_active')) {
    function reeid_license_is_active(): bool
    {
        $key = get_option('reeid_pro_license_key', '');
...
/**
 * Feature-flag gate. In the shim, we only check license presence.
 * Later, we will map $flag to server-provided flags from api.reeid.com.
 *
 * @param string $flag Feature key, e.g. 'elementor_pro_panel', 'bulk_translate_ui'
 * @return bool
 */
if (!function_exists('reeid_feature_enabled')) {
    function reeid_feature_enabled(string $flag): bool
    {
        // Shim policy: if license "active", all PRO flags are allowed.
return (bool) apply_filters('reeid/feature_enabled', reeid_license_is_active(), $flag);
    }
}

/**
 * Optional helper to style UI based on license state.
 */
if (!function_exists('reeid_pro_state_class')) {
    function reeid_pro_state_class(): string
    {
        return reeid_license_is_active() ? 'pro-enabled' : 'pro-disabled';
    }
}
...
/*==============================================================================
  SECTION 12.B — Remote License Verify (HMAC, PHP 7.x safe)
  - Calls https://api.reeid.com/translate with {op:"license.verify"}
  - Verifies X-REEID-Signature using a shared secret (set in wp-config.php)
  - Caches result in a transient and exposes it via a filter so the existing
    reeid_feature_enabled($flag) can use server-provided flags.
==============================================================================*/

if (!defined('REEID_LIC_ENDPOINT')) {
    define('REEID_LIC_ENDPOINT', 'https://api.reeid.com/translate');
}
if (!defined('REEID_LIC_CACHE_TTL_DEFAULT')) {
    define('REEID_LIC_CACHE_TTL_DEFAULT', 12 * HOUR_IN_SECONDS);
}

/**
 * Read the shared HMAC secret for verifying server replies.
 * Set in wp-config.php:
 *   define('REEID_LICENSE_HMAC_SECRET', 'paste-the-base64-secret-here');
 *
 * (We don't store it in the DB; wp-config is better for deployments.)
 */
if (!function_exists('reeid_lic_shared_secret')) {
    function reeid_lic_shared_secret() {
        if (defined('REEID_LICENSE_HMAC_SECRET') && REEID_LICENSE_HMAC_SECRET) {
            return REEID_LICENSE_HMAC_SECRET;
        }
        // Optional filter if you prefer to inject from elsewhere
        $via_filter = apply_filters('reeid/license_hmac_secret', null);
        return is_string($via_filter) ? $via_filter : null;
    }
}

/**
 * Compute base64(HMAC-SHA256(body, secret)), PHP 7.x safe.
 */
if (!function_exists('reeid_hmac_b64')) {
    function reeid_hmac_b64($body, $secret) {
        return base64_encode(hash_hmac('sha256', (string)$body, (string)$secret, true));
    }
}

/**
 * Fetch and cache license status from api.reeid.com.
 * Returns normalized array:
 *   ['valid'=>bool, 'plan'=>string, 'flags'=>array, 'expires_at'=>string|null]
 */
if (!function_exists('reeid_license_status_remote')) {
    function reeid_license_status_remote($force_refresh = false) {
        $tx_key = 'reeid_license_cached_v2';
        if (!$force_refresh) {
        ...
        $secret = reeid_lic_shared_secret();
        // If no secret, fall back to "invalid" (don’t break anything)
        if (!$secret) {
            $data = array(
                'valid'      => false,
            ...
            set_transient($tx_key, $data, MINUTE_IN_SECONDS * 15);
            return $data;
        }

        $license_key = get_option('reeid_pro_license_key', '');
        $site        = home_url('/');
        $payload     = array(
            'op'         => 'license.verify',
        ...
        $res  = wp_remote_post(REEID_LIC_ENDPOINT, $args);
        if (is_wp_error($res)) {
            $data = array(
                'valid'      => false,
        ...
        $sig_srv = (string) wp_remote_retrieve_header($res, 'x-reeid-signature');

        if ($code !== 200 || $body === '' || $sig_srv === '') {
            $data = array(
                'valid'      => false,
                'reason'     => 'bad_http_or_missing_sig',
                'plan'       => 'free',
                'flags'      => array(),
                'expires_at' => null,
                'checked_at' => time(),
        ...
        // Verify HMAC
        $sig_calc = reeid_hmac_b64($body, $secret);
        if (!function_exists('hash_equals') ? ($sig_calc !== $sig_srv) : !hash_equals($sig_calc, $sig_srv)) {
            $data = array(
                'valid'      => false,
                'reason'     => 'sig_mismatch',
                'plan'       => 'free',
        ...
        $ttl = isset($json['cache_ttl']) ? (int)$json['cache_ttl'] : REEID_LIC_CACHE_TTL_DEFAULT;
        $ttl = max(5 * MINUTE_IN_SECONDS, $ttl);

        $data = array(
            'valid'      => !empty($json['valid']),
            'plan'       => isset($json['plan']) ? (string)$json['plan'] : 'free',
            'flags'      => isset($json['flags']) && is_array($json['flags']) ? $json['flags'] : array(),
            'expires_at' => isset($json['expires_at']) ? (string)$json['expires_at'] : null,
            'checked_at' => time(),
        );

        set_transient($tx_key, $data, $ttl);
        return $data;
    }
}

/**
 * Invalidate cache when the license key changes.
 */
add_action('update_option_reeid_pro_license_key', function () {
    delete_transient('reeid_license_cached_v2');
}, 10, 0);

/**
 * Bridge: expose server flags through a filter that our shim can consult.
 * We keep the shim function name intact; it will start honoring this filter
 * once we make it filter-aware (one-line change below).
 */
add_filter('reeid/feature_enabled', function ($default, $flag) {
    $st = reeid_license_status_remote(false);
    if (empty($st['valid'])) {
        return false; // if server says invalid (or we couldn’t verify), block PRO
    }
    // If server provided an explicit flag, use it; otherwise fall back to $default
    if (isset($st['flags'][$flag])) {
        return (bool)$st['flags'][$flag];
    }
    return (bool)$default;
}, 10, 2);
...
if (! function_exists('reeid__s13_license_key')) {
    function reeid__s13_license_key(): string
    {
        ...
        $k = get_option('reeid_pro_license_key', '');
        return reeid__s13_clean_token($k);
    }
}
    ...
    function reeid_elementor_translate_json_s13(int $post_id, string $source_lang, string $target_lang, string $tone = 'Neutral', string $prompt = ''): array
    {
        ...
        $payload = [
            ...
            'options'     => [
                ...
                'prefer_channel' => 'strings',
                'prompt'         => (string) ($prompt ?? ''),
                'system'         => (string) ($prompt ?? ''), // some servers read only "system"
                ...
                // forward Elementor "Custom prompt" to API
                'prompt'         => (string) ($prompt ?? ''),
                'custom_prompt'  => (string) ($prompt ?? ''),
                'user_prompt'    => (string) ($prompt ?? ''),  
                'instructions'   => (string) ($prompt ?? ''),  
            ...
            // these were missing in your broken snippet
            'openai_key'  => reeid__s13_openai_key(),
            'debug'       => ['echo' => true],
        ];

        reeid__s13_log('prompt', [
            'has_top'     => (isset($payload['prompt']) && $payload['prompt'] !== ''),
            'has_options' => (isset($payload['options']['prompt']) && $payload['options']['prompt'] !== ''),
            'has_content' => (isset($payload['content']['prompt']) && $payload['content']['prompt'] !== ''),
            'preview'     => mb_substr((string)$prompt, 0, 80, 'UTF-8'),
        ]);


        $payload['prompt'] = (string) ($prompt ?? ''); // top-level fallback
        if (is_string($prompt) && preg_match('/do\s*not\s*translate\s*:\s*(.+)$/iu', $prompt, $m)) {
            $protect = [];
            foreach (preg_split('/\s*(?:,|;|\band\b)\s*/iu', trim((string)$m[1])) as $t) {
                $t = trim($t, " \t\n\r\0\x0B\"'`.");
                if ($t !== '') {
                    $protect[] = $t;
                }
            ...
            if ($protect) {
                $payload['options']['protect']      = $protect;
                $payload['options']['no_translate'] = $protect; // alias
            }
        }

        $api = reeid__s13_api_base() . '/v1/translate';
        $elem_len = strlen(wp_json_encode($elem, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES));
        reeid__s13_log(
            'req',
            [
                'endpoint'   => $api,
                'editor'     => 'elementor',
                'ok_len'     => strlen((string) $payload['openai_key']),
                'elem_bytes' => $elem_len,
                'strings'    => count($string_map),
            ]
        );
...
/* ---- LEGACY ALIAS ---- */
if (! function_exists('reeid_elementor_translate_json')) {
    /**
     * Elementor → Remote S13 with explicit error surfacing + longer timeouts.
     * Logs to uploads/reeid-client.log and returns server {code,error} to caller.
     *
     * @return array{success:bool, title?:string, excerpt?:string, content?:string, slug?:string, data?:array, error?:string, code?:string}
     */
    function reeid_elementor_translate_json(int $post_id, string $source_lang, string $target_lang, string $tone = 'Neutral', string $prompt = ''): array
    {

        // REEID: Elementor panel Custom Prompt isolation — DO NOT merge with any global prompt.
// Priority: explicit $prompt arg > $_POST['prompt'] > empty.
...
// From here on, use ONLY the Elementor panel prompt:
$prompt = $__elem_prompt;
if (function_exists('reeid_debug_log')) {
    reeid_debug_log('ELEM_PROMPT_ISOLATED', ['len'=>strlen($prompt), 'h'=>md5($prompt)]);
}

        // --- tiny logger to uploads ---
        ...
        $license_key = (string) get_option('reeid_pro_license_key', '');
        ...
        if (!is_array($data)) {
            $log('S13/ELEM_DATA_MISSING', ['post_id' => $post_id]);
            return ['success' => false, 'code' => 'elementor_data_missing', 'error' => 'Elementor JSON missing'];
        }

        // --- Ensure tone is respected even if the remote ignores options.tone ---
        $prompt = (string) ($prompt ?? '');
        $__tone = is_string($tone) ? trim($tone) : '';
        if ($__tone !== '' && strcasecmp($__tone, 'Neutral') !== 0) {
            // Prefix a concise directive; keeps user prompt intact and additive.
            // Avoid duplication if a prior layer already injected a Tone rule.
            if (stripos($prompt, 'tone:') === false) {
                $prompt = "Tone: {$__tone}. Write consistently in this tone.\n" . $prompt;
            }
        }
        ...
        // POST with generous timeouts (Elementor pages can be large)
        ...
        if ($resp === false) {
            $log('S13/CURL_ERROR', ['http' => $http, 'err' => $cerr]);
            return ['success' => false, 'code' => 'curl_error', 'error' => $cerr ?: 'transport_failed'];
        }
        $log('S13/RESP', ['http' => $http, 'bytes' => strlen((string)$resp)]);

        if ($http >= 500) {
            $log('S13/HTTP_5XX', ['http' => $http]);
        ...
        if (!is_array($res)) {
            ...
            if (is_array($maybe)) $res = $maybe;
        }
        if (!is_array($res)) {
            $log('S13/BAD_JSON', ['snippet' => mb_substr($body, 0, 200, 'UTF-8')]);
            return ['success' => false, 'code' => 'bad_response', 'error' => 'non_json_response'];
        }

        // If API emits structured failure, surface it directly
        if (!empty($res['ok']) || !empty($res['success'])) {
            $ok = (bool) ($res['success'] ?? $res['ok'] ?? false);
            if ($ok) {
                $log('S13/OK', ['hasData' => is_array($res['data'] ?? null)]);
                return [
                    'success' => true,
                    'title'   => isset($res['title'])   && is_string($res['title'])   ? $res['title']   : null,
                    'excerpt' => isset($res['excerpt']) && is_string($res['excerpt']) ? $res['excerpt'] : null,
                    'content' => isset($res['content']) && is_string($res['content']) ? $res['content'] : null,
                    'slug'    => isset($res['slug'])    && is_string($res['slug'])    ? $res['slug']    : null,
                    'data'    => isset($res['data'])    && is_array($res['data'])     ? $res['data']    : null,
                ];
            }
        }

        // API said not ok — return its reason so the caller can display it
        $api_code = isset($res['code']) && is_string($res['code']) ? $res['code'] : 'remote_failed';
        $api_err  = isset($res['error']) && is_string($res['error']) ? $res['error'] : 'remote_failed';
        $log('S13/API_FAIL', ['code' => $api_code, 'error' => $api_err]);
...
if (! function_exists('reeid__s15_license_key')) {
    function reeid__s15_license_key()
    {
        ...
        return sanitize_text_field(get_option('reeid_pro_license_key', ''));
    }
}
...
if (! function_exists('reeid_gutenberg_is_non_text_attr')) {
    function reeid_gutenberg_is_non_text_attr(string $attr_key, $attr_val, string $full_path = ''): bool
    {
        ...
        // === Load API rules (minimal local fallback protects IP) ===
        $rules         = reeid_get_gutenberg_nontext_rules();
        $skip_prefixes = $rules['skip_prefixes'] ?? ['style'];
        $skip_exact    = $rules['skip_exact']    ?? ['class','classname','id','url','href'];
        $skip_regex    = $rules['skip_regex']    ?? [];

        // Skip by prefixes (design/meta umbrellas)
        ...
        // Skip by exact keys
        if (in_array($k, $skip_exact, true)) {
            return true;
        }

        // Skip by regex patterns (anchored or fragments), case-insensitive
        foreach ($skip_regex as $rx) {
            $rx = (string) $rx;
            if ($rx === '') continue;
            // Safe delimitering: use ~ and escape any ~ inside the pattern
            $delim = '~';
            $pat   = $delim . str_replace($delim, '\\' . $delim, $rx) . $delim . 'i';
            if (@preg_match($pat, $k)) {
                if (@preg_match($pat, $k) === 1) {
                    return true;
                }
            }
        }
        ...
        // Non-string values are not translatable text
        ...
        // URLs / anchors / protocols
        if (preg_match('/^(https?:|mailto:|tel:|data:|\/\/|#)/i', $v)) {
            return true;
        }

        // CSS color/gradient tokens
        if (preg_match('/^#[0-9a-f]{3,8}$/i', $v)) {
            return true; // hex colors
        }
        if (preg_match('/^(rgb|rgba|hsl|hsla)\s*\(/i', $v)) {
            return true; // rgb/rgba/hsl
        }
        if (stripos($v, 'linear-gradient(') !== false || stripos($v, 'radial-gradient(') !== false) {
            return true;
        }
        if (preg_match('/^var\(\s*[-a-z0-9_]+\s*\)$/i', $v)) {
            return true; // CSS var(--token)
        }

        // Pure numbers or unit values (sizes)
...
/*==============================================================================
  SECTION 19.0: Remote Gutenberg Rules Fetcher (API-backed, minimal fallback)
  - Real rules (textish attribute keys) are served from api.reeid.com
  - Plugin only fetches & caches; fallback is deliberately minimal to avoid IP leak
==============================================================================*/
if (! function_exists('reeid_get_gutenberg_rules')) {
    function reeid_get_gutenberg_rules(): array
    {
        $cache_key = 'reeid_gb_rules_v1';
        $cached    = get_transient($cache_key);
        if (is_array($cached)) {
            return $cached;
        }

        // Minimal fallback — DO NOT put full rules here (protect IP).
        // This intentionally translates far less if API is unreachable.
        $minimal = [
            'textish_keys' => ['content'],
        ];

        // Allow custom endpoints (enterprise self-hosting, etc.)
        $endpoint = apply_filters(
            'reeid_api_rules_endpoint',
            'https://api.reeid.com/v1/walkers/gutenberg-rules.php'
        );
        ...
        if (is_wp_error($res) || (int) wp_remote_retrieve_response_code($res) !== 200) {
            // Flag degraded mode so admins see a gentle notice
            set_transient('reeid_gb_rules_degraded', 1, 6 * HOUR_IN_SECONDS);
            set_transient($cache_key, $minimal, 6 * HOUR_IN_SECONDS);
            return $minimal;
        }

        $body  = wp_remote_retrieve_body($res);
        $json  = json_decode($body, true);
        $rules = is_array($json['rules'] ?? null) ? $json['rules'] : $minimal;

        // Validate & normalize
        if (! isset($rules['textish_keys']) || ! is_array($rules['textish_keys'])) {
            $rules['textish_keys'] = $minimal['textish_keys'];
        }
        $rules['textish_keys'] = array_values(array_unique(array_map('strval', $rules['textish_keys'])));

        // OK path: clear degraded flag and cache full rules for 12h
        delete_transient('reeid_gb_rules_degraded');
...
// Small admin notice if we’re on the minimal fallback
if (! function_exists('reeid_notice_rules_degraded')) {
    function reeid_notice_rules_degraded(): void
    {
        if (! is_admin() || ! current_user_can('manage_options')) {
            return;
        }
        if (! get_transient('reeid_gb_rules_degraded')) {
            return;
        }
        echo '<div class="notice notice-warning is-dismissible"><p>'
           . esc_html__('REEID Translate: Using minimal Gutenberg rules fallback (API unreachable). Translations may cover fewer fields. This is not an error; your site remains functional.', 'reeid-translate')
           . '</p></div>';
    }
    add_action('admin_notices', 'reeid_notice_rules_degraded');
}


/*==============================================================================
  SECTION 19.1: Remote Gutenberg NON-TEXT Rules (API-backed, minimal fallback)
  - Moves the “skip” logic out of the plugin
  - Fallback is intentionally tiny (protect IP), site still works if API is down
==============================================================================*/
if (! function_exists('reeid_get_gutenberg_nontext_rules')) {
    function reeid_get_gutenberg_nontext_rules(): array
    {
        $cache_key = 'reeid_gb_nontext_rules_v1';
        if ($c = get_transient($cache_key)) {
            return is_array($c) ? $c : [];
        }

        $minimal = [
            'skip_prefixes' => ['style'],
            'skip_exact'    => ['class','className','id','url','href'],
            'skip_regex'    => [],
        ];

        $endpoint = apply_filters(
            'reeid_api_nontext_rules_endpoint',
            'https://api.reeid.com/v1/walkers/gutenberg-negative-rules.php'
        );

        $res = wp_remote_get($endpoint, ['timeout' => 5, 'headers' => ['Accept'=>'application/json']]);
        if (is_wp_error($res) || (int) wp_remote_retrieve_response_code($res) !== 200) {
            set_transient('reeid_gb_nontext_rules_degraded', 1, 6 * HOUR_IN_SECONDS);
            set_transient($cache_key, $minimal, 6 * HOUR_IN_SECONDS);
            return $minimal;
        }

        $j = json_decode(wp_remote_retrieve_body($res), true);
        $rules = is_array($j['rules'] ?? null) ? $j['rules'] : $minimal;

        foreach (['skip_prefixes','skip_exact','skip_regex'] as $k) {
            if (!isset($rules[$k]) || !is_array($rules[$k])) $rules[$k] = $minimal[$k];
            $rules[$k] = array_values(array_unique(array_map('strval', $rules[$k])));
        }

        delete_transient('reeid_gb_nontext_rules_degraded');
        set_transient($cache_key, $rules, 12 * HOUR_IN_SECONDS);
        return $rules;
    }
}
...
/*==============================================================================
  SECTION 19.2: Prompt Recipe Fetcher (no content leaves site)
  - Fetches system prompt + JSON schema from api.reeid.com
  - Plugin still calls OpenAI locally with the user's own API key
==============================================================================*/
if (! function_exists('reeid_get_wp_html_recipe')) {
    function reeid_get_wp_html_recipe(string $source_lang, string $target_lang, array $ctx = []): array
    {
        $cache_key = 'reeid_wphtml_recipe_v1';
        if ($c = get_transient($cache_key)) {
            if (is_array($c)) return $c;
        }

        $endpoint = apply_filters(
            'reeid_api_recipe_endpoint',
            'https://api.reeid.com/v1/recipes/wp-html-v1.php'
        );
        ...
        $payload = [
            ...
            'source_lang' => $source_lang,
            'target_lang' => $target_lang,
            'editor'      => (string) ($ctx['editor'] ?? ''),
            'tone'        => (string) ($ctx['tone'] ?? ''),
        ];

        $headers = ['Accept'=>'application/json','Content-Type'=>'application/json'];
        if (function_exists('reeid_get_site_uuid')) $headers['X-REEID-Site'] = (string) reeid_get_site_uuid();
        if (function_exists('reeid_hmac_sig'))     $headers['X-REEID-Sig']  = (string) reeid_hmac_sig(wp_json_encode($payload));
        ...
        // Minimal, non-IP fallback (keeps sites working if API down)
        $fallback = [
            'system'       => 'You are a careful translation engine. Translate user-provided HTML/text from {src} to {dst}. Preserve HTML tags and attributes, do not translate URLs, classes, IDs, or inline CSS tokens. Return strict JSON with fields: title, content, excerpt, slug.',
            'json_schema'  => [
                'type'       => 'object',
        ...
        $recipe = is_array($json['recipe'] ?? null) ? $json['recipe'] : $fallback;

        // Normalize
        if (!isset($recipe['system']) || !is_string($recipe['system'])) $recipe['system'] = $fallback['system'];
        if (!isset($recipe['json_schema']) || !is_array($recipe['json_schema'])) $recipe['json_schema'] = $fallback['json_schema'];

        set_transient($cache_key, $recipe, 12 * HOUR_IN_SECONDS);
        return $recipe;
    }
}
...
if (! function_exists('reeid_gutenberg_walk_and_collect')) {
    function reeid_gutenberg_walk_and_collect(array $blocks, &$map = [], string $prefix = ''): array
    {
       ...
       // Get attribute rules from API (cached). Minimal local fallback hides IP.
$rules = reeid_get_gutenberg_rules();
$textish_keys = $rules['textish_keys'];
...
if (! function_exists('reeid_html_translate_textnodes_fragment')) {
    function reeid_html_translate_textnodes_fragment(string $html, string $source_lang, string $target_lang)
    {
        ...
        foreach ($chunks as $chunk) {
            ...
            $json_rule = "Task: Translate all JSON values FROM {$source_lang} TO {$target_lang}. Keep keys EXACTLY unchanged. Translate ONLY the values. Do not add/remove keys or change JSON. Do not add or remove leading/trailing whitespace in any value. Return STRICT JSON only (no comments/markdown).";
...
if (! function_exists('reeid_translate_via_openai_with_slug')) {
    function reeid_translate_via_openai_with_slug(string $content, string $target_lang, array $ctx = []): array
    {
        ...
        if ($source_lang === $target_lang) {
            ...
            $slug  = sanitize_title(mb_substr(wp_strip_all_tags($title), 0, 60));
    ...
    $json_in,                   // user content = JSON ONLY (no instructions)
    $source_lang,
    $target_lang,
    'gutenberg',
    $tone,                      // keep caller’s tone
    trim($prompt . "\n\n" . $json_rule) // extra rules merged into SYSTEM
);
...
reeid_debug_log(
    ...
    [
        'len'     => strlen($prompt),
        'preview' => function_exists('mb_substr') ? mb_substr($prompt, 0, 80, 'UTF-8') : substr($prompt, 0, 80),
    ]
);
        }
        ...
        if (function_exists('reeid_debug_log')) {
            reeid_debug_log('S18 PROMPT RECEIVED', [
                ...
                'preview' => is_string($prompt) ? (function_exists('mb_substr') ? mb_substr($prompt, 0, 120, 'UTF-8') : substr($prompt, 0, 120)) : '',
                'tone'    => $tone,
            ]);
       ...
       =========================================================== */
        if ($post->post_type === 'product') {
            ...
            $safe_slug  = sanitize_title($slug ?: $safe_title);
            $payload = [
       ...
       =========================================================== */
        ...
        if ($editor === 'elementor') {
           ...
           $slug_source = (string) ($result['slug'] ?? ($result['title'] ?? $post->post_title));
            $slug_final  = function_exists('reeid_sanitize_native_slug')
                ? reeid_sanitize_native_slug($slug_source)
                : sanitize_title($slug_source, '', 'save');
            ...
            // Slug handling
            $translated_slug = '';
            if (isset($title_tr) && is_string($title_tr) && $title_tr !== '') {
                $translated_slug = sanitize_title($title_tr, '', 'save');
            }
            ...
            $new_post = [
                'ID'           => $target_id,
                'post_title'   => !empty($title_tr)   ? $title_tr   : $post->post_title,
                'post_status'  => $publish_mode,
                'post_type'    => $post->post_type,
                'post_name'    => $translated_slug,
                'post_author'  => $post->post_author,
                'post_excerpt' => !empty($excerpt_tr) ? $excerpt_tr : $post->post_excerpt,
            ];
            $target_id = function_exists('reeid_safe_wp_update_post')
                ? reeid_safe_wp_update_post($new_post, true)
                : wp_update_post($new_post, true);
        ...
        if (function_exists('reeid_write_title_all_plugins')) {
            ...
            $final_title = ($editor === 'elementor')
                ...
                : (string)(!empty($title_tr) ? $title_tr : $post->post_title);
            ...
            'post_id' => $target_id,
            'preview' => function_exists('mb_substr') ? mb_substr($final_title_trim, 0, 80, 'UTF-8') : substr($final_title_trim, 0, 80),
        ]);
                    ...
                    if ($desc_tr_trim !== '' && stripos($desc_tr_trim, 'INVALID LANGUAGE PAIR') === false) {
                        ...
                        if (function_exists('reeid_debug_log')) {
                           reeid_debug_log('S18/SEO_SKIPPED_DESC', [
                            'post_id' => $target_id,
                            'preview' => function_exists('mb_substr') ? mb_substr($desc_tr_trim, 0, 160, 'UTF-8') : substr($desc_tr_trim, 0, 160),
                        ]);
...
/* ---------- License / PRO gate (server-side) ---------- */
if ( ! function_exists('reeid_feature_enabled') || ! reeid_feature_enabled('bulk_translate_ui') ) {
    wp_send_json_error(
        array(
            'code'    => 'requires_pro',
            'message' => __('Bulk Translate requires an active REEID license.', 'reeid-translate')
        ),
        403
    );
}
...
$post_id_raw = filter_input(INPUT_POST, 'post_id', FILTER_DEFAULT);
            ...
            foreach ($selected as $lang) {
                ...
                if ($editor === 'elementor' && function_exists('reeid_elementor_translate_json')) {
                    ...
                    $slug_source = (string) ($result['slug'] ?? ($result['title'] ?? $root_post->post_title));
                    $slug_final  = function_exists('reeid_sanitize_native_slug')
                        ? reeid_sanitize_native_slug($slug_source)
                        : sanitize_title($slug_source, '', 'save');
                    ...
                    // Slug uniqueness
                    $slug_final = sanitize_title($title, '', 'save');
                    if ($slug_final === '') $slug_final = $slug_raw;
...
============================================================================================*/
    ...
    function reeid_translate_via_openai_with_slug_legacy(
        ...
        if (empty($translated_slug) && !empty($translated_title)) {
            $translated_slug = sanitize_title($translated_title, '', 'save');
        }
        if (empty($translated_slug)) {
                        ...
                        return oldFetch.apply(this, arguments).then(function(resp) {
                            if (
                                ...
                                resp.clone().json().then(function(data) {
                                    ...
                                    if (/translate.*(already|child).*translation/i.test(message)) {
    ...
    add_filter('wp_nav_menu_objects', function ($items, $args) {
        ...
        foreach ($items as $item) {
            if (is_object($item) && property_exists($item, "object") && property_exists($item, "object_id") && in_array($item->object, ["page", "post"], true) && !empty($item->object_id)) {
                ...
                if ($item_lang === $lang) {
                    $slug = sanitize_title(get_post_field('post_name', $item->object_id));
...
<?php endif; ?>
    ...
    <?php
    // Safe slug fallback: prefer an already-defined $slug, else try common sources ($m or $tr), else empty.
    if (! isset($slug)) {
        if ( isset($m) && is_array($m) && isset($m['slug']) ) {
            $slug = $m['slug'];
        } elseif ( isset($tr) && is_array($tr) && isset($tr['slug']) ) {
            $slug = $tr['slug'];
        } else {
            $slug = '';
        }
    ...
    <tr>
        <th scope="row">
            <p class="description" style="margin-top:6px;">Product name</p>
        </th>
        <td>
            <input type="text"
    ...
    <tr>
        <th scope="row">
            <p class="description" style="margin-top:6px;">Translated Slug</p>
        </th>
        <td>
            <input type="text"
                   id="reeid_tr_<?php echo esc_attr($code); ?>_slug"
                   name="reeid_tr[<?php echo esc_attr($code); ?>][slug]"
    ...
    <tr>
        <th scope="row">
           <p class="description" style="margin-top:6px;">Product short description</p>
        </th>
        <td>
    ...
    <tr>
        <th scope="row">
            <p class="description" style="margin-top:6px;">Product long description</p>
        </th>
        <td>
    ...
    <tr>
        <th scope="row">
            <label for="reeid_tr_<?php echo esc_attr($code); ?>_status">Status</label>
        </th>
        <td>
...
==============================================================================*/
    ...
    /** UI: add a "Delete this translation" */
...
/*==============================================================================
  SECTION 47 : SHORTCODE REGISTRATION & LANGUAGE SWITCHER 
==============================================================================*/
...
==============================================================================*/
 ...
 /*==============================================================================
  ...
  - Allows non-Latin slugs to resolve.
...
==============================================================================*/
    ...
    /* ---------- Libsodium helpers: persist keypair and derive public key (SAFE) ---------- */
...
if (! function_exists('reeid_get_or_make_kp_b64')) {
    /**
     * Returns [keypair_b64|null, pub_b64|null].
     * - Persists a new keypair when none exists or when stored value is invalid.
     * - Accepts older installs that stored a secret key only and tries to derive a pub key.
     * - Always defensive; avoids throwing SodiumException on malformed stored data.
     */
    function reeid_get_or_make_kp_b64(): array
    {
        // If libsodium missing, be graceful and return nulls.
        if (! reeid_sodium_have()) {
            error_log('reeid: libsodium not available; cannot create/derive keypair');
            return [null, null];
        }

        // Option name (must match your plugin constant)
        if (! defined('REEID_OPT_KP_SECRET')) {
            // Fallback option name if constant missing; adjust if needed
            define('REEID_OPT_KP_SECRET', 'reeid_kp_secret');
        }

        $kp_b64 = (string) get_option(REEID_OPT_KP_SECRET, '');

        // Helper: create, persist and return new keypair + pub
        $make_and_store = function () {
            try {
                $kp = sodium_crypto_box_keypair();
                $kp_b64_new = base64_encode($kp);
                update_option(REEID_OPT_KP_SECRET, $kp_b64_new, true);
                $pub = sodium_crypto_box_publickey($kp);
                return [$kp_b64_new, base64_encode($pub)];
            } catch (\Throwable $e) {
                error_log('reeid: failed to generate/store keypair: ' . $e->getMessage());
                return [null, null];
            }
        };

        // No stored value -> make + store
        if ($kp_b64 === '') {
            error_log('reeid: no stored keypair found; generating new keypair');
            return $make_and_store();
        }

        // Decode safely
        $kp_bin = base64_decode($kp_b64, true);
        if ($kp_bin === false || $kp_bin === null) {
            error_log('reeid: stored kp option not valid base64; regenerating keypair');
            return $make_and_store();
        }

        $len = strlen($kp_bin);

        // Case A: full keypair stored
        if (defined('SODIUM_CRYPTO_BOX_KEYPAIRBYTES') && $len === SODIUM_CRYPTO_BOX_KEYPAIRBYTES) {
            try {
                $pub = sodium_crypto_box_publickey($kp_bin);
                return [$kp_b64, base64_encode($pub)];
            } catch (\Throwable $e) {
                error_log('reeid: sodium_crypto_box_publickey failed on stored keypair: ' . $e->getMessage());
                return $make_and_store();
            }
        }

        // Case B: secret key only
        if (defined('SODIUM_CRYPTO_BOX_SECRETKEYBYTES') && $len === SODIUM_CRYPTO_BOX_SECRETKEYBYTES) {
            $pub = null;
            try {
                if (function_exists('sodium_crypto_box_publickey_from_secretkey')) {
                    $pub = sodium_crypto_box_publickey_from_secretkey($kp_bin);
                } elseif (function_exists('sodium_crypto_scalarmult_base')) {
                    $pub = sodium_crypto_scalarmult_base($kp_bin);
                }
            } catch (\Throwable $e) {
                error_log('reeid: pub-from-secret derivation failed: ' . $e->getMessage());
                $pub = null;
            }

            if (is_string($pub) && strlen($pub) === SODIUM_CRYPTO_BOX_PUBLICKEYBYTES) {
                // Try to create a full keypair and persist if possible
                try {
                    if (function_exists('sodium_crypto_box_keypair_from_secretkey_and_publickey')) {
                        $kp_full = sodium_crypto_box_keypair_from_secretkey_and_publickey($pub, $kp_bin);
                        $kp_b64_new = base64_encode($kp_full);
                        update_option(REEID_OPT_KP_SECRET, $kp_b64_new, true);
                        return [$kp_b64_new, base64_encode($pub)];
                    } else {
                        // No constructor available — return secret+derived pub (do not persist new kp)
                        error_log('reeid: keypair constructor not available; returning secret+derived-pub without storing full keypair');
                        return [$kp_b64, base64_encode($pub)];
                    }
                } catch (\Throwable $e) {
                    error_log('reeid: failed to build/persist keypair from secret+pub: ' . $e->getMessage());
                    return $make_and_store();
                }
            }

            // Could not derive pub — regenerate
            error_log('reeid: stored secret key present but unable to derive public key; regenerating keypair');
            return $make_and_store();
        }

        // Unknown/garbage length — regenerate
        error_log('reeid: stored kp option has unexpected length (' . $len . '); regenerating keypair');
        return $make_and_store();
    }
}
    ...
    if (! function_exists('reeid_api_handshake')) {
        ...
        function reeid_api_handshake(bool $force = false): array
        {
            ...
            $payload = [
                'license_key'    => trim((string) get_option('reeid_pro_license_key', 'REPLACE_ME')), // adjust to your stored license option
                'site_url'       => home_url(),
 ...
 * ============================================================ */
    if (! function_exists('reeid_validate_openai_key')) {
        ...
        function reeid_validate_openai_key()
        {
            ...
            check_ajax_referer('reeid_translate_nonce_action', '_wpnonce');
...
/*====================================================================================================
   SECTION 55 : Final adjustments
 * Non-AJAX: Delete single-language inline translation for a product.
 * Endpoint (used by the "Delete this translation" button):
 *  wp-admin/admin-post.php?action=reeid_wc_delete_translation&post={ID}&lang={lang}&_wpnonce={nonce}
 ====================================================================================================*/
add_action('admin_post_reeid_wc_delete_translation', function () {
    ...
    // Read and validate incoming params (accept GET and POST for compatibility)
    $post_id = isset($_REQUEST['post']) ? absint($_REQUEST['post']) : 0;
    $lang_raw = isset($_REQUEST['lang']) ? wp_unslash($_REQUEST['lang']) : '';
    $nonce = isset($_REQUEST['_wpnonce']) ? wp_unslash($_REQUEST['_wpnonce']) : '';

    if (! $post_id || $lang_raw === '') {
        wp_die( esc_html__('Missing product or language.', 'reeid-translate'), 400 );
    }

    // Verify nonce created earlier in the metabox: wp_create_nonce('reeid_del_tr')
    if (! wp_verify_nonce( $nonce, 'reeid_del_tr' )) {
        wp_die( esc_html__('Security check failed (nonce).', 'reeid-translate'), 403 );
    }

    // Capability check for this specific post
    if (! current_user_can('edit_post', $post_id)) {
        wp_die( esc_html__('Insufficient permissions to edit this product.', 'reeid-translate'), 403 );
    }

    // Sanitize language code (keep same logic your helper expects)
    $lang = strtolower( substr( trim( (string) $lang_raw ), 0, 20 ) );

    // Call the helper that removes the inline payload/meta (if present)
    $deleted = false;
    if ( function_exists('reeid_wc_delete_translation_meta') ) {
        try {
            $deleted = (bool) reeid_wc_delete_translation_meta( $post_id, $lang );
        } catch ( Exception $e ) {
            $deleted = false;
        }
    }

    // Redirect back to referrer if available, otherwise to product edit screen
    $redirect = wp_get_referer();
    if ( ! $redirect ) {
        $redirect = admin_url( 'post.php?post=' . $post_id . '&action=edit' );
    }

    // Add query args so admin notices can show success/failure
    $redirect = add_query_arg(
        array(
            'reeid_wc_deleted'      => $deleted ? '1' : '0',
            'reeid_wc_deleted_lang' => rawurlencode( $lang ),
        ),
        $redirect
    );
...
/**
 * Show inline admin notice after delete redirect.
 */
add_action('admin_notices', function() {
    if (! isset($_GET['reeid_wc_deleted']) ) return;

    $ok = intval( $_GET['reeid_wc_deleted'] );
    $lang = isset($_GET['reeid_wc_deleted_lang']) ? wp_unslash( $_GET['reeid_wc_deleted_lang'] ) : '';

    if ($ok) {
        printf(
            '<div class="notice notice-success is-dismissible"><p>%s</p></div>',
            esc_html( sprintf( __('Translation for %s removed.', 'reeid-translate'), $lang ?: __('<unknown>', 'reeid-translate') ) )
        );
    } else {
        printf(
            '<div class="notice notice-error is-dismissible"><p>%s</p></div>',
            esc_html( sprintf( __('Failed to remove translation for %s.', 'reeid-translate'), $lang ?: __('<unknown>', 'reeid-translate') ) )
        );
    }
});
 ...
 * - Languages source = reeid_get_supported_languages(), narrowed to en,de,fr,zh
 * ==========================================================================*/
if (!function_exists('reeid_hreflang_print_canonical')) {
    function reeid_hreflang_print_canonical() {
        if (!is_singular('product')) return;

        // Build language set from canonical function
        $all = function_exists('reeid_get_supported_languages')
             ? array_keys(reeid_get_supported_languages())
             : array('en','de','fr','zh');

        // Keep cluster lean + ensure FR present
        $preferred = array('en','de','fr','zh');
        $langs = array_values(array_intersect($all, $preferred));
        if (empty($langs)) $langs = array('en');

        // Ensure default in set
        ...
        if (function_exists('reeid_hreflang_render')) {
            ...
            if ($snippet) {
                echo $snippet;
                // Respect the bridge’s “already echoed” guard if present
    ...
    add_action('wp', function () {
        ...
        // Hook our canonical printer at the same priority
        add_action('wp_head', 'reeid_hreflang_print_canonical', 90);
    }, 0);
}
/* REEID Hreflang Shim — force hook + marker */
add_action('init', function () {
    // Hook our printer; it bails on non-products
    if (!has_action('wp_head', 'reeid_hreflang_print_canonical')) {
        add_action('wp_head', 'reeid_hreflang_print_canonical', 90);
    }
    // Visual marker just before our printer
    add_action('wp_head', function () {
        if (is_singular('product')) { echo "<!-- REEID-HREFLANG-SHIM -->\n"; }
    }, 89);
});
/* REEID_SHIM_MUTED: prefer bridge as sole product hreflang printer */
add_action('init', function () {
    remove_action('wp_head', 'reeid_hreflang_print_canonical', 90);
}, 99);
...
if (!function_exists('reeid_hreflang_dedupe_buffer')) {
    function reeid_hreflang_dedupe_buffer($html) {
        ...
        // Keep the LAST occurrence of each code (bridge block is usually last/correct)
        $map = [];
        for ($i = 0; $i < count($m[0]); $i++) {
            $code = strtolower($m[1][$i]);
            if (!isset($map[$code])) { $map[$code] = $m[0][$i]; }
        }
...
/* === REEID shim: normalize enabled-languages option for the bridge ========== */
if (!defined('REEID_LANGS_PREOPT_SHIM')) {
    define('REEID_LANGS_PREOPT_SHIM', true);
    add_filter('pre_option_reeid_enabled_languages', function ($pre) {
        global $wpdb;
        // Read raw stored value without triggering get_option recursion
        $raw = $wpdb->get_var(
            $wpdb->prepare("SELECT option_value FROM {$wpdb->options} WHERE option_name=%s LIMIT 1", 'reeid_enabled_languages')
        );
        if ($raw === null) return '["en"]';

        $val = maybe_unserialize($raw);
        if (is_array($val)) {
            // Bridge wants JSON string
            return json_encode(array_values($val));
        }
        if (is_string($val) && preg_match('/^\s*\[.*\]\s*$/', $val)) {
...
// --- REEID hreflang bridge ---
if ( file_exists( __DIR__ . '/includes/reeid-hreflang-bridge.php' ) && ! defined( 'REEID_HREFLANG_BRIDGE_LOADED' ) ) {
    require_once __DIR__ . '/includes/reeid-hreflang-bridge.php';
}

/* BEGIN REEID: include admin UI fixes */
if ( file_exists( __DIR__ . '/includes/admin-ui-fixes.php' ) ) {
    require_once __DIR__ . '/includes/admin-ui-fixes.php';
}


/* =============================================================
 * SECTION: WC Hreflang Override (inline model, enabled-langs)
 * - Scope: products only; posts/pages unaffected
 * - Strategy: remove original hreflang hook, add our own
 * ============================================================= */
add_action('init', function () {
    // If original printer isn’t there, nothing to override
    if (!function_exists('reeid_hreflang_print_canonical')) return;

    // Try to detach original at default priority 10
    remove_action('wp_head', 'reeid_hreflang_print_canonical', 10);

    // Our printer (products only)
    add_action('wp_head', function () {
        if (!is_singular('product')) return;

        // Pull enabled language codes from main plugin settings
        $enabled = function_exists('reeid_get_enabled_languages') ? (array) reeid_get_enabled_languages() : array();
        $enabled = array_values(array_filter(array_map('strval', $enabled)));

        // Default language fallback
        $default = get_option('reeid_default_lang', 'en');
        if (empty($enabled)) $enabled = array($default);

        // Normalize codes (e.g., hr, nl, my, de, zh-cn)
        $enabled = array_values(array_unique(array_map(function ($l) {
            $l = strtolower(trim($l));
            return preg_replace('/[^a-z0-9\-]/i', '', $l);
        }, $enabled)));

        $pid = get_queried_object_id(); if (!$pid) return;
        $self_url = get_permalink($pid); if (!$self_url) return;

        if (!function_exists('reeid_build_lang_prefixed_url')) return;

        $pairs = array();
        foreach ($enabled as $code) {
            $url = reeid_build_lang_prefixed_url($self_url, $code, $default);
            if ($url) $pairs[$code] = $url;
        }

        // x-default -> default language URL
        $xd = reeid_build_lang_prefixed_url($self_url, $default, $default);
        if ($xd) $pairs['x-default'] = $xd;
...
/* =============================================================
 * REEID — WC hreflang (FINAL2, priority 0)
 * - Products only; reads option `reeid_enabled_languages` (JSON)
 * - Emits x-default; standalone URL builder fallback
 * - Marker: <!-- REEID-WC-HREFLANG FINAL2 [...] -->
 * ============================================================= */
add_action('wp_head', function () {
    if (!is_singular('product')) return;

    // Enabled languages from option (fallback to helper if empty)
    $raw = get_option('reeid_enabled_languages', '[]');
    $arr = is_string($raw) ? json_decode($raw, true) : (array)$raw;
    if (!is_array($arr)) $arr = [];
    if (empty($arr) && function_exists('reeid_get_enabled_languages')) {
        $arr = (array) reeid_get_enabled_languages();
    }

    // Normalize + ensure default present
    $default = get_option('reeid_default_lang', 'en');
    $enabled = array_values(array_unique(array_map(function($l){
        $l = strtolower(trim((string)$l));
        return preg_replace('/[^a-z0-9\-]/i','',$l);
    ...
    $pid = get_queried_object_id(); if (!$pid) return;
    $self = get_permalink($pid);    if (!$self) return;

    // URL builder (prefer helper; else safe prefix)
    $build = function(string $url, string $code, string $default){
        if (function_exists('reeid_build_lang_prefixed_url')) {
            return reeid_build_lang_prefixed_url($url, $code, $default);
        }
        $p = wp_parse_url($url);
        if (!$p || empty($p['path'])) return $url;
        $path = $p['path']; if ($path === '' || $path[0] !== '/') $path = '/'.$path;

        if ($code === $default) {
            $np = $path;
        } else {
            if (preg_match('~^/([a-z]{2}(?:-[a-z]{2})?)(/|$)~i', $path)) {
                $np = preg_replace('~^/([a-z]{2}(?:-[a-z]{2})?)(/|$)~i', '/'.$code.'$2', $path, 1);
            } else {
                $np = '/'.$code.$path;
            }
        }
        ...
        $frag  = isset($p['fragment']) && $p['fragment']!=='' ? '#'.$p['fragment'] : '';
        return $scheme.'://'.$host.$port.$np.$query.$frag;
    };

    $pairs = [];
    foreach ($enabled as $code) {
        $u = $build($self, $code, $default);
        if ($u) $pairs[$code] = $u;
    }
    $xd = $build($self, $default, $default);
    if ($xd) $pairs['x-default'] = $xd;

    if (!$pairs) return;

    echo "<!-- REEID-WC-HREFLANG FINAL2 ".esc_html(wp_json_encode(array_keys($pairs)))." -->\n";
    foreach ($pairs as $code => $u) {
        echo "<link rel=\"alternate\" hreflang=\"".esc_attr($code)."\" href=\"".esc_url($u)."\" />\n";
    }
}, 0);
// REEID-PLUG-CLEAN loader
if ( file_exists(__DIR__.'/includes/reeid-hreflang-normalize.php') ) { require_once __DIR__.'/includes/reeid-hreflang-normalize.php'; }

/* REQ: bundled hreflang enforcer (load now, not via hook) */
if ( file_exists(__DIR__ . '/includes/rt-hreflang-enforcer.php') ) {
    require_once __DIR__ . '/includes/rt-hreflang-enforcer.php';
}
require_once __DIR__ . '/includes/reeid-woo-slug-ui.php';

// Load inline slug helpers (moved from mu-plugins)
if ( file_exists( __DIR__ . '/includes/inline-slug.php' ) ) {
    require_once __DIR__ . '/includes/inline-slug.php';
}


/**
 * Bridge: allow admin-ajax.php POST -> reuse existing admin_post deletion logic.
 * Adds: wp_ajax_reeid_wc_delete_translation
 * Place: anywhere inside the plugin (after helper functions like reeid_wc_delete_translation_meta exist).
 */
add_action('wp_ajax_reeid_wc_delete_translation', function () {

    // must be logged in (same guard as admin_post handler)
    if (! is_user_logged_in()) {
        wp_send_json_error('Not logged in', 403);
    }

    // accept POST payload (AJAX)
    $post_id = isset($_POST['post']) ? absint($_POST['post']) : 0;
    $lang_raw = isset($_POST['lang']) ? wp_unslash($_POST['lang']) : '';
    // accept nonce in several common param names (security, _ajax_nonce, _wpnonce)
    $nonce = $_REQUEST['security'] ?? $_REQUEST['_ajax_nonce'] ?? $_REQUEST['_wpnonce'] ?? '';

    if (! $post_id || $lang_raw === '') {
        wp_send_json_error('Missing product or language', 400);
    }

    // verify nonce (use the same action name your plugin expects)
    if (! $nonce || ! wp_verify_nonce($nonce, 'reeid_wc_delete_translation')) {
